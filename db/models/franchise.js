        ...
        isValidDigitalElements(value) {
          ...
          if (typeof value === "string") {
            try {
              ...
              console.log("Converted to array:", value); // ['computer', 'printer']

              // Assign the array back to digitalElements
              this.digitalElements = value;
            } catch (error) {
              throw new Error(
                "Digital elements must be a valid JSON array or list"
              );
            }
          }

          // Validate for non-empty array when panCenter is true
          if (this.panCenter && value.length === 0) {
            throw new Error(
              "Digital elements must be a non-empty array when Pan Center is true"
            );
          }
        },
      },
